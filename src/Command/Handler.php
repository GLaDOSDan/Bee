...
class Handler
{
    ...
    public function download($args)
    {
        ...
        if (!isset($args[1])) {
            ...
            if (strlen(str_replace(range(0, 9), '', substr($args[0], -3))) == strlen(substr($args[0], -3))) {
                ...
                $bytes = self::bytes_multiplier(substr($args[0], 0, -3), substr($args[0], -3));
            } elseif (!is_numeric(substr($args[0], -2))) {
                // Last two characters are non-numeric, so they're probably a multiplier
                $bytes = self::bytes_multiplier(substr($args[0], 0, -2), substr($args[0], -2));
            } else {
            ...
            $bytes = self::bytes_multiplier($amount, $args[1]);
        }
    ...
    public function loadTorrentAnnounce()
    {
        ...
        $this->TorrentAnnounce->set_infohash($this->info_hash);
        $this->TorrentAnnounce->set_size($this->size);

        $this->TorrentAnnounce->set('info_hash', pack('H*', $this->info_hash));
     ...
     * @return string
     */
    public static function bytes_multiplier($amount, $multiplier)
    {
